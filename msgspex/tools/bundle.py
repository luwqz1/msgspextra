...
from msgspex.tools.annotations import Annotations, MappingAnnotations
...
class Bundle[R]:
    ...
    def args(self) -> tuple[typing.Any, ...]:
        return tuple(self.context[name] for name in resolve_posonly_arg_names(self.function, start_idx=self.start_idx) if name in self.context)
    ...
    def __and__(self, other: object, /) -> typing.Self:
        ...
        if self.function != other.function:  # type: ignore
            raise ValueError(
...
def get_default_args(func: AnyFunction, /) -> dict[str, typing.Any]:
    ...
    return {key: value for sublist in (parameters["args"], parameters["kwargs"]) for key, value in sublist if value is not inspect.Parameter.empty}
