...
type RoutineMethodType = types.MethodType | types.MethodDescriptorType | types.MethodWrapperType | types.BuiltinMethodType
type RoutineDescriptorType = types.MethodDescriptorType | types.GetSetDescriptorType

_BUILTINS: typing.Final[frozenset[typing.Any]] = frozenset(x for name in dir(builtins) if getattr((x := getattr(builtins, name)), "__module__", None) == "builtins")
...
def fullname(obj: typing.Any, /) -> str:
    ...
    module = builtins.__name__ if _is_builtin(obj) and module != builtins.__name__ else _module_name(sys.modules[module])
